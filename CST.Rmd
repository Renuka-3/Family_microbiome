...
```{r initCST_CLR, message=F,echo=FALSE, warning=FALSE}
library("phyloseq")
library("ggplot2")
library("cluster")
library("igraph")
...
```{r transform-data_CLR, message=FALSE, echo=FALSE, warning=FALSE}
# Init / Read data
# Was created with: source("create_phyloseq.R")
ps <- readRDS("inputdata/processed/phyloseq/physeqnew.RDS")
temp.phy <- filter_taxa(ps, function (x) {sum(x > 0) > 1}, prune=TRUE)
tt <- data.frame(tax_table(temp.phy))
ps <- transform(temp.phy, "compositional")
```


# Cluster into CSTs

```{r MDS_CLR, warning=FALSE, message=FALSE, echo=FALSE, warning=FALSE}
...
```{r PCoA-cutoff2_CLR, warning=FALSE, message=FALSE, echo=FALSE, warning=FALSE}
...
```{r gap-stat_CLR, message=FALSE, echo=FALSE, warning=FALSE}
...
```{r pam-k5_CLR, message=FALSE, echo=FALSE, warning=FALSE}
K <- 2 
...
```{r see-pam-k2_CLR,eval=T,results = 'hide',  message=FALSE, echo=FALSE, warning=FALSE}
